...
@app.get("/index.jsx", include_in_schema=False)
def block_source_entry_file():
    """
    In production, Vite must load built /assets/*.js files only.
    If /index.jsx is requested, it means the original source entry script
    leaked into the built HTML or browser cache. Return 404 instead of
    serving index.html as JavaScript.
    """
    raise HTTPException(status_code=404, detail="Source entry file is not served in production.")
...
def serve_frontend(full_path: str):
    ...
    API routes stay API routes. Real static file paths should not receive
    index.html, because browsers would try to parse HTML as JS/CSS.
    """
